 ...
 * @param {String} property 需要更新菜单数据
 */
...
/**
 * @description 获取传递过来节点的一级节点
 * @param {Array} arr 整个菜单数据
 * @param {String} node 增加的节点 
 */
// 获取节点的父级节点（一级节点）
...
/**
 * @description 设置角色拥有的菜单
 * @param {Array} tree  数据源
 * @param {Object} nodes 设置角色拥有菜单权限的数据
 * @param {String} property 'checked'
  ...
  menu.forEach((item) => {
    ...
    properties.forEach((property) => {
      if (item[property] && item[property].length > 0) {
        ...
        //   const tmp = item[property][index];
        //   if (tmp.checked || tmp.__titleIsFunction__) {
        //     arr.push(tmp._id)
        //   }
        //   if (tmp[property] && tmp[property].length > 0) {
        //     arr.push(getPropertyIds(tmp[property], properties))
        //   }
        // }
      }
    })
